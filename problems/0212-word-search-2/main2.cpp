...
private:
    ...
    /**
     * Invariants:
     * 1. curr_node is not null and has a non-zero count (we ensure this in the root call
     *   by running dfs as long as the trie is non-empty)
     * 2. buffer string corresponds to the curr_node and the char at (r, c) is not part of the buffer
     *   and is the next potential char after the buffer.
     * 3. After the dfs call, buffer will be restored to its value before the call.
     * 4. Same with the visited state of r, c. It will be restored to its value before the call.
     *
     * Not invariants:
     * 1. (r, c) need not be a valid cell
     * 2. (r, c) could already be visited
     * 3. The character at (r, c) need not be a valid child link to the given trie node
     *
     * The decision of this invariant is taken instead of having the curr_node (& the buffer)
     * including the character at (r, c) is so that the code becomes much cleaner, especially
     * in the first call of dfs inside the nested for loop.
     * This approach has the tradeoff of unnecessary function calls on out of bounds (r, c)
    ...
    {
        // If the cell is out of bounds, we can skip it
        if (r < 0 || m <= r)
            return;
        if (c < 0 || n <= c)
            return;
        ...
        curr_node = curr_node->children[char_rc - 'a'];
        ...
        if (curr_node->n)
        {
            // Visit all the neighbours of the current cell
            for (int i = 0; i < 4; i++)
                dfs(board, buffer, r + drs[i], c + dcs[i], m, n, trie, curr_node, result);
        }

        // Unmark this cell as visited because we are backtracking and we are done with the dfs call
        // on the current cell
        board[r][c] = buffer.back();
        buffer.pop_back();
    }
        ...
        const std::vector<std::string> &words)
    {
         ...
         * 5. Cleaner code compared to main1.cpp solution. Read the invariants of dfs call.
         */
        ...
        for (int r = 0; r < m && !trie.empty(); r++)
            for (int c = 0; c < n && !trie.empty(); c++)
                dfs(board, buffer, r, c, m, n, trie, root, result);
