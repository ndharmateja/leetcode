...
private:
     ...
     *
     * Lets say S is the set of subsets that can be formed from nums[idx+1:]
     * Then the subsets of nums[idx:] is the union of set S and S' which is
     * the set formed by adding nums[i] to each of the elements of S
     * S can be found recursively
     *
     * Eg: nums = [A, B, C]
     * then S = subsets formed by nums[1:] = {{}, {B}, {C}, {B, C}}
     * then S' = nums[i] added to each of the elements of S = {{A}, {A, B}, {A, C}, {A, B, C}}
     * subsets formed by nums = S U S' = {{}, {B}, {C}, {B, C}, {A}, {A, B}, {A, C}, {A, B, C}}
     */
                          ...
                          std::vector<std::vector<int>> &result)
    {
        ...
        // Add nums[i] to each of the elements in the result
